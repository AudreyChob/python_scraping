...
def write_author_details():
    """
        cette fonction ecrit les details de chaques auteurs dans un fichier excel
    """
    ...
    for author in author_list:
        auteur = author.string.replace(' ', '-').replace('.','').replace("é", "e")
        url_page_author = 'http://quotes.toscrape.com/author/'+auteur+'/'
        page_author = urllib.request.urlopen(url_page_author)
        ...
        author_born_location = author_details.find('span', class_="author-born-location")
        author_descript = author_details.find('div', class_="author-description")
        if author_born.contents != []:
            ...
            work_sheet['C'+str(i)] = author_born_location.contents[0]
            work_sheet['D'+str(i)] = author_descript.contents[0]
            i += 1
